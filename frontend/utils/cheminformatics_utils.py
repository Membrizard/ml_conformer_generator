...
def prepare_speck_model(mol: Chem.Mol, reference: Chem.Mol = None):
    ...
    if reference:
        ...
        for j, atom in enumerate(reference.GetAtoms()):
            ...
            mol_json['atoms'].append({'symbol': 'Ref', 'x': position.x, 'y': position.y, 'z': position.z})

        for bond in reference.GetBonds():
            mol_json['bonds'].append({'begin_atom': bond.GetBeginAtomIdx() + last_atom_idx, 'end_atom': bond.GetEndAtomIdx() + last_atom_idx})
