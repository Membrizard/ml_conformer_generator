...
for key in new_state_dict.keys():
    if key in old_weights:
        # If the key exists in the old weights, map it
        mapped_state_dict[key] = old_weights[key]
        print(f"Mapped weight for {key}")
    else:
        print(f"Warning: {key} not found in old weights!")

generative_model.load_state_dict(mapped_state_dict, strict=False)
torch.save(generative_model.state_dict(), "new_compilable_edm_moi_chembl_15_39.weights")