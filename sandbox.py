import torch
from models import EGNNDynamics, EquivariantDiffusion
...
def generate_sample(device,
    ...
    batch_context = normed_context.unsqueeze(0).repeat(
        batch_size, 1
    )

    batch_context = batch_context.unsqueeze(1).repeat(1, max_n_nodes, 1) * node_mask

    x, h = generative_model.sample(batch_size, max_n_nodes, node_mask, edge_mask, batch_context, fix_noise=fix_noise)
...
samples = generate_sample(device,
               generative_model,
               ref_context,
