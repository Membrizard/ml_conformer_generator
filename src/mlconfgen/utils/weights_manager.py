...
logger = logging.getLogger("mlconfgen_trainer.weights_manager")
...
class WeightsManager:
    ...
    def __init__(
        ...
        self.cache_dir = Path(
            cache_dir
            or os.getenv("MLCONFGEN_CACHE", Path.home() / ".mlconfgen" / "cache")
        )
    ...
    def _file_lock(self, lock_path: Path, timeout=60):
        ...
        while lock_path.exists():
            if time.time() - start > timeout:
                raise TimeoutError(
                    f"[MLConfGen] Timeout waiting for lock {lock_path}"
                )
            time.sleep(0.2)
    ...
    def resolve(self, force_download: bool = False) -> Path:

        p = Path("adj_mat_seer.pt")
        if p.exists():
            return p

        cache_path = self.cache_dir / "adj_mat_seer.pt"
        lock_path = cache_path.with_suffix(".lock")

        cache_path.parent.mkdir(parents=True, exist_ok=True)

        if cache_path.exists() and not force_download:
            logger.info(f"[MLConfGen] Using cached weights")
            return cache_path
        ...
        with self._file_lock(lock_path):
            ...
            logger.info(f"[MLConfGen] Downloading weights ...")

            try:
                with requests.get(self.url, headers=self._headers(), stream=True) as r:
                    r.raise_for_status()
                ...
                logger.error(f"[MLConfGen] Download failed due to: {e}")

            logger.info(f"[MLConfGen] Download complete")
    ...
    def clear_cache(self) -> None:
        ...
        logger.info("[MLConfGen] Weights cache cleared: %s", self.cache_dir)
