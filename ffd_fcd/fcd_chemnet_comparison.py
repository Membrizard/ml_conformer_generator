...
class MolecularGraphEncoder(nn.Module):
    ...
    def forward(self, atomic_nums, adjacency_matrix):
        ...
        x = self.bn1(x.permute(0, 2, 1)).permute(0, 2, 1)  # (batch_size, max_atoms, hidden_dim)
        
        # Second layer
        x_in = x
        message = torch.bmm(adjacency_matrix, x_in)  # Message passing
        x = F.relu(self.conv2(message))
        x = self.bn2(x.permute(0, 2, 1)).permute(0, 2, 1)  # (batch_size, max_atoms, hidden_dim)
        
        # Third layer
        x_in = x
        message = torch.bmm(adjacency_matrix, x_in)  # Message passing
        x = F.relu(self.conv3(message))
        x = self.bn3(x.permute(0, 2, 1)).permute(0, 2, 1)  # (batch_size, max_atoms, hidden_dim)
...
def load_molecules_from_sdf(filename):
    ...
    if len(embeddings) == 0:
        ...
        print(f"Successfully loaded {len(embeddings)} ChemNet embeddings from {filename}")
...
def load_molecules_from_csv(filename):
    ...
    if embeddings:
        print(f"Successfully loaded {len(embeddings)} ChemNet embeddings from CSV {filename}")
        print(f"Embedding dimension: {embeddings[0].shape}")
...
def calculate_and_print_fcd(name1, embeddings1, name2, embeddings2):
    ...
    if len(embeddings1) > 3 and len(embeddings2) > 3:
        try:
            ...
            print(f"FCD between {name1} ({len(embeddings1)} molecules) and {name2} ({len(embeddings2)} molecules): {fcd_value:.4f}")
...
def main():
    ...
    for i in range(len(file_names)):
        for j in range(i+1, len(file_names)):
            name1 = file_names[i]
            name2 = file_names[j]
            result = calculate_and_print_fcd(name1, all_embeddings[name1], name2, all_embeddings[name2])
            if result[4] is not None:  # If FCD was successfully calculated
                results.append(result)
    
    # Save results to CSV
    if results:
        result_df = pd.DataFrame(results, columns=['Set1', 'Set2', 'Size1', 'Size2', 'FCD'])
